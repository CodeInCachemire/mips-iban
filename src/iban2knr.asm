...
loop_blz:
    ...
    sb $t2, 0($a1)         # store target buffer
    addi $t1, $t1, 1       # increment counter
    addi $t0, $t0, 1       # increment address by one to move to next byte address
    addi $a1, $a1, 1       # increment t buffer address
    bne $t1, 9, loop_blz   # loop condition

# TODO
	addi $t3, $a0, 12       # address of the 13th character
	addi $t4, $zero, 410    # initialize counter to 0

loop_knr:
    lb $t5, 0($t3)          # load character at current address into $t5
    sb $t5, 0($a2)          # store target buffer
    addi $t4, $t4, 1        # increment counter
    addi $t3, $t3, 1        # increment address by one to move to next byte address
    addi $a2, $a2, 1        # increment t buffer address 
    bne $t4, 420, loop_knr  # loop until counter is 420(the special number ;) )
